...
class Decoder:
    def __init__(self):
        ...
        self.ANALYTICS_PENDING_CONTAINER = os.getenv(
            "ANALYTICS_PENDING_CONTAINER"
        )
        self.icds = None
        self.package = None
    ...
    def download_icds(self):
        try:
            ...
            zip_ref = zipfile.ZipFile(self.QARDirIn + "/ICDs.zip")  # create zipfile object
    ...
    def download_package(self):
        try:
            ...
            with open(file=(self.ScriptsDirIn + "/QAR_Decode.zip"), mode="wb") as sample_blob:
                download_stream = blob_client.download_blob()
                sample_blob.write(download_stream.readall())
            print("Package retrieved", flush=True)
            zip_ref = zipfile.ZipFile(self.ScriptsDirIn + "/QAR_Decode.zip")  # create zipfile object
            zip_ref.extractall(self.ScriptsDirIn)  # extract file to dir
            zip_ref.close()  # close file
            os.remove(self.ScriptsDirIn + "/QAR_Decode.zip")  # delete zipped file
            #self.flatten(self.ScriptsDirIn + "/for_redistribution_files_only", self.ScriptsDirIn)
    ...
    def decode(self, airline, tail):
        ...
        #QAR_Decode.initialize_runtime(["-nojvm", "-nodisplay"])
        ...
        try:
            ...
            print("Error decoding: ", e, flush=True)
            self.rollback()
    ...
    def install_package(self):
        ...
        res = subprocess.run(["python", "setup.py", "install"])
        print("exit code: ", res)
        

if __name__ == "__main__":
    decoder = Decoder()
    schedule.every(10).seconds.do(decoder.read_from_queue)
